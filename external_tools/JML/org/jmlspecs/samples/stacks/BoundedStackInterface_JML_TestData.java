// @(#)$Id: BoundedStackInterface_JML_TestData.java,v 1.3 2004/01/22 20:01:54 leavens Exp $
 ...
 *
 */
public abstract class BoundedStackInterface_JML_TestData
    extends junit.framework.TestCase
{
    /** Initialize this class. */
    public BoundedStackInterface_JML_TestData(java.lang.String name) {
        super(name);
    }
    ...
    public junit.framework.TestSuite overallTestSuite() {
        return new junit.framework.TestSuite("Overall tests for BoundedStackInterface");
    }
    ...
    //@ ensures \fresh(\result);
    protected org.jmlspecs.jmlunit.strategies.IndefiniteIterator
        vorg_jmlspecs_samples_stacks_BoundedStackInterfaceIter
        (java.lang.String methodName, int loopsThisSurrounds)
    {
        return vorg_jmlspecs_samples_stacks_BoundedStackInterfaceStrategy.iterator();
    }

    /** The strategy for generating test data of type
     * org.jmlspecs.samples.stacks.BoundedStackInterface. */
    private org.jmlspecs.jmlunit.strategies.StrategyType
        vorg_jmlspecs_samples_stacks_BoundedStackInterfaceStrategy
        = new org.jmlspecs.jmlunit.strategies.NewObjectAbstractStrategy()
            {
                protected Object make(int n) {
                    BoundedStackInterface receiver;
                    switch (n) {
                    case 0:
                        receiver = new BoundedStackImplementation();
                        return receiver;
                    case 1:
                        receiver = new BoundedStackImplementation();
                        receiver.push(new int[] { 3, 4 });
                        return receiver;
                    case 2:
                        receiver = new BoundedStackImplementation();
                        for (int j = 0; j < receiver.getSizeLimit(); j++) {
                            receiver.push(new Integer(j));
                        }
                        return receiver;
                    case 3:
                        receiver = new BoundedStackImplementation();
                        for (int j = 0; j < receiver.getSizeLimit()-1; j++) {
                            receiver.push(new Integer(10-j));
                        }
                        return receiver;
                    case 4:
                        receiver = new BoundedStack();
                        return receiver;
                    case 5:
                        receiver = new BoundedStack(1);
                        return receiver;
                    case 6:
                        receiver = new BoundedStack(1);
                        receiver.push(new int[] { 3, 4 });
