// @(#)$Id: charExtensibleStrategy.java-generic,v 1.5 2005/12/24 21:20:31 chalin Exp $
 ...
 */
public abstract class CharExtensibleStrategy
    extends CharAbstractStrategy
{
    /** The data to return in the iterations */
    private /*@ nullable @*/ char[] data = null;  //@ in objectState;
    ...
    protected char[] getData() {
        char[] defd = defaultData();
        char[] ret = defd;
        char[] added = addData();
        if (added.length != 0) {
            ret = new char[defd.length + added.length];
