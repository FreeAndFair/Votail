// @(#)$Id: byteExtensibleStrategy.java-generic,v 1.5 2005/12/24 21:20:31 chalin Exp $
 ...
 */
public abstract class ByteExtensibleStrategy
    extends ByteAbstractStrategy
{
    /** The data to return in the iterations */
    private /*@ nullable @*/ byte[] data = null;  //@ in objectState;
    ...
    protected byte[] getData() {
        byte[] defd = defaultData();
        byte[] ret = defd;
        byte[] added = addData();
        if (added.length != 0) {
            ret = new byte[defd.length + added.length];
